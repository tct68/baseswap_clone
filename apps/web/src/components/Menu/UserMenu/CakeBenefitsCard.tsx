...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>{t(`bTW allows you to boost your yield in PancakeSwap Farms by up to 2x.`)}</Text>
  ...
  const {
    ...
    <>
      <Text>
        {t(`vTW boosts your voting power to %totalScore% in the PancakeSwap voting governance.`, {
