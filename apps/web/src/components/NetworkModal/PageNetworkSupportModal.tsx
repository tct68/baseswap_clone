import { Button, Modal, Text, Grid, Box, Message, MessageText } from '@baseswap/uikit'
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        ...
        <Text small>
          {t(
            'Our Pools, Limit, Trading Competition, Prediction and NFTs features are currently available only on BNB Chain! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.BASE_GOERLI) : switchNetworkAsync(ChainId.BASE_GOERLI))}
          >
